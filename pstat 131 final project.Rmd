...
spotify$skipped<-as.factor(spotify$skipped)
...
set.seed(2435)
spotify_fold <- vfold_cv(spotify_train, strata = skipped, v = 10, repeats = 5, na.rm= TRUE)
...
lda_fit
qda_fit <- fit(lda_wkflow, spotify_train)
qda_fit
```
...
spotify_log<-predict(log_fit, new_data = spotify_train, type = "prob")

spotify_log_col<-bind_cols(spotify_log, spotify_train)
log_reg_acc<- augment(log_fit, new_data= spotify_train) %>% accuracy(truth=skipped, estimate=.pred_class)
log_reg_acc
...
#boxplot of skipped and songs
ggplot(data = spotify, mapping = aes(y = skipped, x = track_name, fill = track_name)) + 
  geom_boxplot()+                     
  theme(legend.position = "none", axis.text.x = element_text(size = 8, angle = 60, hjust = 1))+   # remove legend (redundant)+
  labs(title = "Boxplot of Skipped by Track Name", x = '')  
```
